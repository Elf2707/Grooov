...
public class GroooovPlayer extends BasicPlayer {
	...
	//player states
	private enum playerStateType { PAUSE, PLAY, STOP };
	...
	public void play() {
		try {
			...
			if( playerState == playerStateType.PAUSE){
				...
				File songFile = new File( lstSongsList.getModel().getElementAt(songIndex).getFilename());
				super.stop();
				super.open(songFile);
				super.play();
	...
	public void playNext(){
		...
		if(playSongIndex == ((DefaultListModel<Mp3File>)lstSongsList.getModel()).size() - 1){
			playSongIndex = 0;
		}else{
			playSongIndex++;
		}
		lstSongsList.setSelectedIndex(playSongIndex);
		play(((DefaultListModel<Mp3File>)lstSongsList.getModel()).getElementAt(playSongIndex));
	}
	...
	public void playPrev(){
		...
		if(playSongIndex == 0){
			playSongIndex = ((DefaultListModel<Mp3File>)lstSongsList.getModel()).size() - 1;
		}else{
			playSongIndex--;
		}
		lstSongsList.setSelectedIndex(playSongIndex);
	 ...
	 * @param volumeLevel - level of volume
	 */
