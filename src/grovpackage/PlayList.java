...
public class PlayList extends JList<Mp3File> implements Serializable  {
	...
	public void setCellRenderer(
			ListCellRenderer<? super Mp3File> cellRenderer) {
		super.setCellRenderer(cellRenderer);
	}
   
   //Add song to List
   public void addSong(File songFile) throws Exception {
	  if( !songFile.exists() ) throw new IOException("PlayList:add:Файл не существует");
   ...
   public void loadList(File loadFile){
	   ...
	   while( addFileName != null ){
		 try {
			...
			e.printStackTrace();
			System.out.println("Error adding songs from loadin PlayList " + loadFile.getAbsolutePath());
		 }
		 try {
			   addFileName = inputFile.readLine();
		   }catch (IOException e1) {
   ...
   public void delSong(){
	  ...
	  if(delIndex == -1){
		  JOptionPane.showMessageDialog(getTopLevelAncestor(), "Where is no song selected!!!");
		  return;
	  }
